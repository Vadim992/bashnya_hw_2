...
// Я создаю массив (приоритетеную очередь), отсротированный потприоритетам обработки флагов: cli.QueueFlags
// флаги обрабатываются в слудющей последовательности: i, f, s, затем остальные
// массив copyLines содержит копию элементов оригинального массива (оригинальный массив - массив строк входных данных,
// один элемент этого массива - это строка во входных данных)

// "Эталонный вид" массива - это массив состоящий из строк в которых нет пробелов вокруг (слева и справа), а внутри слова разделены одним пробелом
...
// trimSpace trims space around ands inside string (use d this func to create canonical slise of string)
